...
newtype Final = Final (forall r. Interpreter (R4 r) (R4 r))

unFinal :: Final -> (forall r. Interpreter (R4 r) (R4 r))
unFinal (Final i) = i

foreign import final :: Final

foreign import base :: SpecT (Run (R1 ())) Unit Identity Unit

finalFunction :: Run (R4 ()) ~> Run (R4 ())
finalFunction = unInterpreter (unFinal final)

pipeline :: Run (R1 ()) ~> Run (R4 ())
pipeline a =
  a
    # unInterpreter (unEff eff >>> unEnv env >>> unAff aff)
    # finalFunction
