...
impl IndexingPairs {
    pub fn data_constructors(&mut self, id: TypeItemId) -> impl Iterator<Item = TermItemId> {
        self.data_constructors.iter().filter_map(
            move |(type_id, term_id)| if *type_id == id { Some(*term_id) } else { None },
        )
    }

    pub fn class_members(&mut self, id: TypeItemId) -> impl Iterator<Item = TermItemId> {
