...
test_each_file! { in "./crates/lexing_new/tests/lexer" => |content: &str| {
    use std::fmt::Write;
...
test_each_file! { in "./crates/lexing_new/tests/lexer" as reconstruction => |content: &str| {
...
mod invariants {
    ...
    fn trailing_qualifier() {
        let lexed = lexing_new::lex("Hello.");
