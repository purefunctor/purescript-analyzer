...
impl PatternConstructor {
    ...
    ///
    /// Only [`PatternConstructor::DataConstructor`] has a non-zero arity.
    pub fn arity(&self) -> usize {
        if let PatternConstructor::DataConstructor { fields, .. } = self { fields.len() } else { 0 }
    }

    /// Returns the fields of this pattern constructor.
    ///
    /// Only [`PatternConstructor::DataConstructor`] should have fields.
    pub fn fields(&self) -> &[PatternId] {
        if let PatternConstructor::DataConstructor { fields, .. } = self { fields } else { &[] }
    }
    ...
    ///
    /// For [`PatternConstructor::DataConstructor`], this function overrides the
    /// fields. Otherwise, the fields must be empty as enforced by an assertion.
    ///
...
fn specialise_vector(
    ...
    if let PatternKind::Wildcard = first_column.kind {
        if let PatternConstructor::DataConstructor { fields, .. } = expected {
    ...
    // Only DataConstructor has fields to splat
    match constructor {
