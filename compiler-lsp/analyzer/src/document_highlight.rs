...
use lowering::{BinderId, ExpressionId, ExpressionKind, LetBindingNameGroupId, TermVariableResolution};
...
fn highlight_expression(
    ...
    let kind = lowered
        .info
        .get_expression_kind(expression_id)
    ...
    {
        match resolution {
            TermVariableResolution::Binder(binder_id) => highlight_binder(engine, current_file, *binder_id),
            TermVariableResolution::Let(let_binding_id) => highlight_let(engine, current_file, *let_binding_id),
...
pub fn implementation(
                ...
                for node in token.parent_ancestors() {
                    if let Some(eq) = cst::LetBindingEquation::cast(node.clone()) {
                        if let Some(eq_id) = stabilized.lookup_cst(&eq)
                            ...
                            && let Some(highlights) =
                                highlight_let(engine, current_file, group_id)?
                        {
                            return Ok(Some(highlights));
                        }
                    }
    ...
    let Some(locations) = crate::references::implementation(engine, files, uri.clone(), position)? else {
        return Ok(None);
    };
